---
title: "All Toti WGS Samples with kraken2 on kraken2 Standard Database"
author: "Balázs Kakuk"
date: "2024-10-20"
output:
  html_document:
...
writetables  <- T
save.Figs    <- F
res.dir      <- 'Toti_kraken2_standard'; dir.create(res.dir)
...
## Filtering samples?
filter.samples.pattern <- NULL #'FR'  ### 
...
- Host-removed and QC-d reads were compared to Kraken 2 Standard Database
- Unclassified reads *were* excluded
- Taxa with total reads number of 10 or lower were excluded 
- Eukaryotes *were NOT* excluded from the analysis
...
ps.all <- readRDS("Toti_Illumina_WGS_kraken2_Standard_PS.rds")
...
## Summary of Read Counts and Classification Status
```{r}
metadata <- data.frame(sample_data(ps.all))
...
unclass.summary <- rbind(
  ## Classified Reads
  otutab[ grepl('unassigned|unclassified', rownames(otutab), ignore.case = T), ],
  ## Unclassified Reads
  colSums(otutab[!grepl('unassigned|unclassified', rownames(otutab), ignore.case = T), ]),
  ## All Reads
  otutab[ grepl('unassigned|unclassified', rownames(otutab), ignore.case = T), ] + colSums(otutab[!grepl('unassigned|unclassified', rownames(otutab), ignore.case = T), ]),
  ## Ratio of Classified Reads to All Reads
  colSums(otutab[!grepl('unassigned|unclassified', rownames(otutab), ignore.case = T), ]) / 
    (otutab[ grepl('unassigned|unclassified', rownames(otutab), ignore.case = T), ] + colSums(otutab[!grepl('unassigned|unclassified', rownames(otutab), ignore.case = T), ]))
)
rownames(unclass.summary) <- c('Classified Reads', 'Unclassified Reads', 'Total_Read_Count', 'Classification_Ratio')

unclass.summary <- data.table(Variable=rownames(unclass.summary), unclass.summary)

unclass.summary

unclass.summary <- merge(melt(unclass.summary, value.name = 'ratio', variable.name = 'sample'), metadata, by='sample')
```

### Summary of Total Read Counts 
```{r}

ggplot(unclass.summary[Variable == 'Total_Read_Count'], aes(sample, ratio)) + 
  geom_col(aes(fill=platform), position = 'dodge', color='black') +
...
### Summary of Classification ( *Classified* / *All Reads* )
```{r}

ggplot(unclass.summary[Variable == 'Classification_Ratio'], aes(sample, ratio)) + 
  geom_col(aes(fill=platform), position = 'dodge', color='black') +
  scale_fill_manual(values = pal.man) +
  scale_y_continuous(limits = c(0, 1), labels = function(x) paste0(round(x*100, 2), ' %')) +
  coord_flip() +
  theme_ipsum() +
  facet_wrap(~sample, ncol=2, scales = 'free_y')
...
if (exclude.unclass) {
  ...
  otutab <- otutab[!grepl('unassigned|unclassified', rownames(otutab), ignore.case = T), ]
  taxtab <- taxtab[!grepl('unassigned|unclassified', rownames(taxtab), ignore.case = T), ]
...
```{r, Figure 2, fig.height = fig.height.vr, fig.width = fig.width.vr, eval=multiV[2]}

samples.to.keep <- rownames(metadata[ metadata$DNA.Isolation.Kit %in% c('ZMB') &
                                     (metadata$platform == 'ONT' | metadata$platform == 'NovaSeq')
                                     , ])

ps <- prune_samples(samples.to.keep, ps.glom.norm.top.spec)
...
ggsave(paste0('Article_Figures', '/', 'Figure_3_Kraken2.png'), ggtop20, width = 7, height = 14)
...
scatplot <- function(bray.df) {
  ggplot(bray.df, 
    ...
    geom_point(aes(shape      = source, #sequencing_date,
                   color      = DNA.Isolation.Kit), size=2) +
    geom_text_repel(aes(label = sample_name, color=DNA.Isolation.Kit)) +
...
scatplot <- function(bray.df) {
  ggplot(bray.df, 
         aes(NMDS1, NMDS2)) +
    geom_point(aes(shape      = DNA.Isolation.Kit, #sequencing_date,
                   color      = source), size=2) +
