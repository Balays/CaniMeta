---
title: "Zymo D6300; D6331; and D6323 Comparison : EMU on EMU database"
author: "Balázs Kakuk"
date: "2024-10-20"
...
```{r, keep taxa from D3600}
ps      <- ps.all
otutab  <- data.frame(ps@otu_table)

otutab  <- apply(otutab, 2, function(x) x / sum(x))
otutab  <- otutab * count_multiplier
otutab  <- apply(otutab, 2, function(x) round(x,0))

ps@otu_table <- otu_table(as.matrix(otutab), taxa_are_rows = T)

#taxa.D3600 <- c("Limosilactobacillus fermentum", "Bacillus subtilis", "Staphylococcus aureus", "Listeria monocytogenes",
#                "Salmonella enterica", "Escherichia coli", "Enterococcus faecalis", "Pseudomonas aeruginosa")
#ps    <- prune_taxa(taxa.D3600, ps)

ps    <- prune_taxa(zymo.GS$species, ps)
ps.standard <- ps

```


```{r, correct metadata D6331}



GS.otutab   <- data.frame(otu_table(ps))
GS.taxtab   <- data.frame(tax_table(ps))
GS.metadata <- data.frame(sample_data(ps))

#
GS.metadata[GS.metadata$sample_name == GS_name, 'primer'] <- GS_name
sample_data(ps) <- sample_data(GS.metadata)
#
...
```{r}
primers <- unique(GS.metadata$primer)[-1]

de.seq <- phyloseq_to_deseq2(ps, ~primer)
de.seq <- DESeq(de.seq,  test = "Wald", fitType = "parametric", sfType = "poscounts" )

coldata <- as.data.frame(de.seq@colData)

rowdata <- data.frame(assay(de.seq))

res.deseq <- data.frame(NULL)
for (i in primers) {
  res <- results(de.seq, contrast=c('primer', GS_name, i))
  res <- as.data.frame(res)
  res$comparison <- paste(GS_name, i, sep = '_VS_')
  res$taxon <- rownames(res)
  
  res.deseq <- plyr::rbind.fill(res, res.deseq)
...
```{r}
# Convert DESeq results to data.table
setDT(res.deseq)

# Reshape data to have log2FoldChange values for each taxon across comparisons
heatmap_data <- dcast(res.deseq, taxon ~ comparison, value.var = "log2FoldChange")
setDF(heatmap_data)

# Set taxon names as rownames and remove the taxon column for heatmap plotting
...
pheatmap(heatmap_data, 
         ...
         show_colnames = TRUE, 
         scale = "row", 
         main = "Heatmap of log2 Fold Difference",
         fontsize_row = 8, 
         fontsize_col = 10)
```


```{r, fig.height = 5, fig.width = 7, eval=T}

res.deseq[,grp1 := gsub('_VS.*', '', comparison)][,grp2 := gsub('.*_VS_', '', comparison)]

#res.deseq <- merge(res.deseq, GS.metadata)

# Plot the heatmap using ggplot2
p <- ggplot(res.deseq, aes(x = taxon, y = grp2, fill = log2FoldChange)) +
  ...
  scale_fill_gradient2(#low = "#d73027", mid = "white", high = "#4575b4",
                       ...
                       limit    =  c(-2,3), #range(res.deseq$log2FoldChange, na.rm = TRUE), # c(-3, 3),
                       name="log2 Fold Difference") +
  scale_fill_viridis(option = "C", name = "log2 Fold Difference", limits = c(-2, 3)) +
  coord_flip() +
  theme_minimal() +
...
## Number of significantly different taxa
```{r}
res.deseq[,is.significant := fifelse(log2FoldChange >= 2 | log2FoldChange <= -2 & padj <= 0.05 , T, F)]

sig.frq <- res.deseq[,.N,by=.(grp2, is.significant)]

sig.frq <- dcast(sig.frq, grp2 ~ is.significant)
sig.frq


fwrite(sig.frq,    paste0(res.dir, '/sig.frq.tsv'),     sep='\t')
fwrite(res.deseq,  paste0(res.dir, '/l2F_diff.dt.tsv'), sep='\t')
