...
fwrite(det.sum, 'detection_metrics.tsv', sep = '\t')
...
fit1 <- euler(list(
    ...
    `Canis lupus familiaris`    = unique( up_dt[`Canis_lupus`  == T,  get(t)] )
))

p1 <- plot(
  fit1,
  quantities = TRUE,
  fills = list(fill = c("Fecal Reference" = "#1f77b4", "Homo sapiens" = "#ff7f0e", "Canis lupus familiaris" = "#005B2A"), alpha=0.6),
  edges = list(col  = c("Fecal Reference" = "black",   "Homo sapiens" = "black",   "Canis lupus familiaris" = "black"  ), lwd = 1)
)
...
fwrite(taxa.pres, 'FR_H.sapiens_C.lupus_taxa.presence.tsv',    sep ='\t')
