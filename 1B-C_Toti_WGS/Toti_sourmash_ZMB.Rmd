...
writetables  <- T
save.Figs    <- T
res.dir      <- 'Toti_sourmash_ZMB'; dir.create(res.dir)
...
## Database
db <- 'sourmash'
db.dir <- '../databases/sourmash_NCBI'
...
## Filtering samples?
filter.samples.pattern <- 'ZMB' ## NULL #'FR'  ### 
...
add.GS.taxa <- NULL ## 40 ## OR: 'all' -->> add all GS taxa 
...
## How many taxa per level to plot?
top <- 25
...
if (db == 'proGcontigs_2') {
  ...
  #db.data <- fread('../databases/sourmash_2022/lineage_summary.tsv')
  #saveRDS(db.data, '../databases/sourmash_2022/lineage_summary.rds') 
  db.data <- readRDS('../databases/sourmash_2022/lineage_summary.rds') 
  
  db.uni.data <- unique(db.data[,..ranks])
  rm(db.data)
} else if (db == 'kraken2') {
  NULL # for now ...
}
...
ps.all <- readRDS("Toti_all_WGS_sourmash.k31_NCBI_PS.rds")
...
```{r}
## Filter for samples
if(!is.null(filter.samples.pattern)) {
  samples.to.keep <- c(
    sample_names(ps.all)[
      grepl(filter.samples.pattern,   sample_names(ps.all))]
  )
...
```{r}
setDT(metadata)
metadata[,source := 'Toti']
metadata[,group        := .GRP, by=.(source, DNA.Isolation.Kit, Vregion, platform, workflow, db)] 
...
if (exclude.unclass) {
  ...
  otutab <- otutab[!grepl('unassigned', rownames(otutab), ignore.case = T), ]
  taxtab <- taxtab[!grepl('unassigned', rownames(taxtab), ignore.case = T), ]
  
  otutab <- otutab[!grepl('unclassified', rownames(otutab), ignore.case = T), ]
  taxtab <- taxtab[!grepl('unclassified', rownames(taxtab), ignore.case = T), ]
...
plotfun <- function(ps, x='sample_name', t=t, fill=t) {
  plot_bar(ps, x=x, fill=fill
    ...
    facet_grid(cols = vars(platform, DNA.Isolation.Kit), scales = 'free')
...
```{r, Glom taxa at Superkingdom-level}
n <- 1
...
```{r, Superkingdom-level plot, fig.height=fig.height.vr, fig.width = fig.width.vr, eval=multiV[2]}
...
ps.glom.norm.top.sking <- ps.glom.norm.top
ps.glom.norm.sking     <- ps.glom.norm
ps.glom.top.sking      <- ps.glom.top
ps.glom.sking          <- ps.glom
...
## Figure 3A: *Compare ONT WGS and NovaSeq on Toti sample nr. 2 isolated with Zymo MagBead*
 
```{r, fig.height = fig.height.vr, fig.width = fig.width.vr, eval=multiV[2]}

i  <- 2
t  <- ranks[i]
ps <- prune_samples(samples.to.keep, ps.glom.norm.top.phy)



my.pal <- colors_for_PS(ps, rank=t, pal.man)
...
ggtop20    <- plotfun(
  ...
  #+ ggtitle(paste0('Top', top, ' ', t))
  facet_nested_wrap(~platform, nrow = 1, scales = 'free') 
...
otutabx  <- rbind(1 - colSums(otutab)); row.names(otutabx)  <- 'other'
otutab   <- rbind(otutab, otutabx)

taxtabx  <- data.frame(taxon = 'other', row.names = 'other' ); colnames(taxtabx) <- t
taxtab   <- rbind(taxtab,  taxtabx) 
...
ps.data <- dt.from.ps(ps); ps.data[, taxon := get(t)]
ps.data[,taxon := factor(taxon, levels = rev(names(my.pal)))]

ggtop20p   <- ggplot(ps.data, aes(count, platform)) +
  geom_col(aes( fill = taxon), color = 'black') +
  scale_fill_manual(values=my.pal) +
  xlim(c(0,1)) + 
  guides(fill = guide_legend(direction = "vertical", ncol = 1, title = 'Phylum')) +
    theme_ipsum() + 
    theme(legend.position="right", 
          legend.text = element_text(face = "italic"),
          panel.spacing.x = unit(1.5, 'mm'),
...
ggsave(paste0('Article_Figures', '/', 'Figure_3A_ONT_vs_Illumina_WGS_sourmash_top20_barplot.png'), ggtop20, width = 7, height = 14)

ggtop20 + coord_flip() # ggpubr::rotate()
...
taxa.pres <- dt.from.ps(ps.all, 
                        glom_taxa    = T, t = t, 
                        glom_samples = T, summarise.samples = 'platform')

taxa.pres <- taxa.pres[sum_count > 0, ]
cols <- c(t, 'platform', 'sum_count')
taxa.pres <- unique(taxa.pres[, ..cols])

taxa.pres <- dcast.data.table(taxa.pres, ... ~ platform, value.var = 'sum_count', fill = NA)
...
ggsave(paste0(res.dir, '/', t, '_overlap_UpSet.png'), p1,
       width = 12, height = 9)
...
### Euler
fit1 <- euler(list(
    `ONT`     = unique( up_dt[`ONT`     == T,  get(t)] ),
    `NovaSeq` = unique( up_dt[`NovaSeq` == T,  get(t)] )
))

p1 <- plot(
  fit1,
  quantities = TRUE,
  fills = list(fill = c("ONT"="#1f77b4", "NovaSeq"="#ff7f0e"), alpha=0.6),
  edges = list(col  = c("ONT"="black",   "NovaSeq"="black"), lwd = 1)
)

p1

ggsave(paste0(res.dir, '/', t, '_overlap_Euler.png'), p1,
       width = 12, height = 9)
saveRDS(p1, paste0(res.dir, '/', t, '_overlap_Euler.rds'))
...
taxa.pres <- dcast.data.table(taxa.pres, ... ~ platform, value.var = 'sum_count')
...
## Figure 3: *Compare ONT WGS and NovaSeq on Toti sample nr. 2 isolated with Zymo MagBead*
...
dir.create('Article_Figures')
ggsave(paste0('Article_Figures', '/', 'Figure_3B_ONT_vs_Illumina_WGS_sourmash_Euler.png'), Fig3B, width = 7, height = 14)
saveRDS(Fig3B, paste0('Article_Figures', '/', 'Figure_3B_ONT_vs_Illumina_WGS_sourmash_Euler.rds') )
...
Fig3 <- plot_grid(Fig3A, Fig3B, nrow = 1, rel_widths = c(3,1), labels = c('a', 'b'))

dir.create('Article_Figures')
ggsave(paste0('Article_Figures', '/', 'Figure_3B_Final.png'), Fig3, width = 21, height = 12, bg = 'white')
