---
title: "All Toti WGS Samples with kraken2 on kraken2 Viral Database"
author: "Balázs Kakuk"
date: "2024-10-20"
output:
  html_document:
...
writetables  <- T
save.Figs    <- F
res.dir      <- 'Toti_kraken2_viral_db'; dir.create(res.dir)
...
- Host-removed and QC-d reads were compared to Kraken 2 Viral Database
...
ps.all <- readRDS("Toti_Illumina_WGS_kraken2_Viral_PS.rds")
...
ggplot(unclass.summary[Variable == 'Classification_Ratio'], aes(sample, ratio)) + 
  geom_col(aes(fill=platform), position = 'dodge', color='black') +
  scale_fill_manual(values = pal.man) +
  scale_y_continuous(limits = c(0, 0.01), labels = function(x) paste0(round(x*100, 2), ' %')) +
  coord_flip() +
  theme_ipsum() +
  facet_wrap(~sample, ncol=2, scales = 'free_y')
...
# Relative Abundance of Viruses at Each Taxonomic Level
...
# taxa.to.keep <- rownames(taxtab[taxtab$superkingdom == 'Viruses', ])
# ps <- prune_taxa(taxa.to.keep, ps.all)
...
## Supplementary Figure S3: *Compare Viruses in all Toti WGS data*
...
ggsave(paste0('Article_Figures', '/', 'SuppFigure_3.png'), ggtop20, width = 12, height = 16)
